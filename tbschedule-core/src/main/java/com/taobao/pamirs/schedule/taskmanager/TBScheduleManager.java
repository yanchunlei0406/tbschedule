...
/**
 * 1、任务调度分配器的目标： 让所有的任务不重复，不遗漏的被快速处理。<br>
 * 2、一个Manager只管理一种任务类型的一组工作线程。<br> 
 * 3、在一个JVM里面可能存在多个处理相同任务类型的Manager，也可能存在处理不同任务类型的Manager。<br>
 * 4、在不同的JVM里面可以存在处理相同任务的Manager<br> 
 * 5、调度的Manager可以动态的随意增加和停止<br>
 * <p>
